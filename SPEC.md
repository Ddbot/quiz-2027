...
- **NFR-007 — [Privacy / Data residency]** Personal data SHALL be stored only in the EU: the Supabase project in an EU region, and every Durable Object created through a namespace pinned to the `eu` jurisdiction. *(REQ §5.4, §11.19)* — **PoC status:** DB half **unmet** — the production Supabase project is in `eu-west-2` (London, UK), accepted by owner decision for test-only data (see Q-009). DO half tracked by Q-001.
...
| Q-009 | Production Supabase project (`dehwczlcnmhmtfarrxnv`) was created in **`eu-west-2` (AWS London, United Kingdom)** during MILESTONE-01, not an EU-member region. NFR-007 ("stored only in the EU") is therefore **not strictly met** for the database. Owner decision (2026-09-10): accept for the zero-cost PoC — test data only, and the UK holds an EU data-adequacy decision. Before any real personal data is processed, either migrate the project to an EU-member region (`eu-central-1`, `eu-west-1`, `eu-west-3`, `eu-north-1`, …) or record UK adequacy reliance in the registre des traitements. | Owner / DPO | Open — NFR-007 (DB half) unmet by owner decision; revisit before commercial phase |
