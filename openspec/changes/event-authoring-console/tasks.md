...
- [x] 11.2 Push the migration to production (`supabase db push`) after explicit owner confirmation — verify the bucket and its policies exist in the production project (e.g. via the Supabase dashboard or a scoped query), and record confirmation before merging. Owner confirmed via AskUserQuestion ("Yes, push to production"); `supabase db push` applied `20260914203234_event_media_storage.sql` successfully; verified with `supabase storage ls --linked --experimental`, which lists `event-media/` on the production project.
