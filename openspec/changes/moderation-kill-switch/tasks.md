...
- [x] 9.1 No destructive migration this milestone — confirm production deploy succeeds on merge, then (after explicit owner confirmation) a real connection test: trigger and clear the kill switch against a live production event, hide/rename a participant or team and confirm it disappears from the console's expectations and the big screen's rankings while its answers remain queryable in Postgres. **Result**: `verify`/`Deploy worker` both green on the `main` merge (PR #30). Unlike MILESTONE-09/10, this milestone's migration (`moderate_participant`/`moderate_team`) is additive, not zero — it hadn't been pushed to production yet, so the owner's first moderation attempt 404'd; fixed with an explicit, owner-confirmed `supabase db push` (only the one new migration applied, no destructive change). After that, the owner confirmed hide/rename and the kill switch all worked correctly against a live production event.
