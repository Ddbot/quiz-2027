...
- [x] 10.2 FR traceability table:
  ...
  **Deploy-config fix found during PR verification**: this is the first milestone where `apps/web` imports anything from `@quiz/shared` directly (`useEventRoom.ts`, `LiveGameView.tsx`). Vercel's `apps/web/vercel.json` `buildCommand` was `pnpm --filter web build`, which never built `@quiz/shared` first — passed every prior milestone's Vercel deploy by coincidence (no prior `apps/web` file imported it), then failed this PR's preview deploy with `TS2307: Cannot find module '@quiz/shared'`. Fixed to `pnpm --filter @quiz/shared build && pnpm --filter web build` (pnpm's `--filter "...web"` upstream-dependency shorthand was tried first but empirically only matched `web` itself in this pnpm 12.3.4 workspace, not its `@quiz/shared` dependency — the explicit two-step command was used instead, verified locally against a clean `dist/`).
