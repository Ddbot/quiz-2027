...
- [x] 8.1 No new Postgres migration this milestone — confirm `apps/party`'s production deploy succeeds on merge, then (after explicit owner confirmation) a real connection test against the deployed Worker/console: reveal at least one step, show the leaderboard, then end the event, confirming `event_final_participant`/`event_final_team` rows land in production Postgres, the event becomes read-only, and the screen can still be driven to the podium view afterward. **Result**: `verify`/`Deploy worker` both green on the `main` merge (PR #28). Owner ran the flow against a disposable production event with no joined players: `mc:reveal` and `mc:show_leaderboard` correctly produced empty results/leaderboard (0 participants, not an error); `mc:end` transitioned `event.status` to `ended` with a real `ended_at`, and `event_final_participant`/`event_final_team` queries ran cleanly against the now-`ended` event (0 rows, matching the empty roster — the point was confirming the finalisation path executes without error, not exercising nonzero scores); `operator:display` to the podium view still worked post-end.
