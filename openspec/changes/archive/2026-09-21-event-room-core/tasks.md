...
- [x] 10.2 Confirm the three production worker secrets (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_JWKS_URL`) are actually set correctly in the deployed Worker's secrets. `wrangler secret list` confirms presence (all four secrets exist). Merged PR #17 and #18 (the latter fixing a `deploy-worker.yml` gap PR #17's merge exposed — see below); the automatic `wrangler deploy` on merge to `main` succeeded and is live. Attempted a real WebSocket connection test against the deployed Worker: a garbage token was correctly rejected in 460ms with code 4001 (proves the Worker is live and running the new MILESTONE-05 code, not the old echo baseline), but a full end-to-end test (real JWT through JWKS verification + Postgres role resolution) could not be completed — production has anonymous sign-ins disabled (`anonymous_provider_disabled`) and requires email confirmation for self-service signup, and I have no production service-role access to create/confirm a test account myself. User confirmed (AskUserQuestion): stop here — secret presence + the garbage-token rejection is sufficient evidence for this task; the full path remains unverified pending a real token, to be exercised naturally once MILESTONE-06+ wires up real connections.

  **Also discovered during this task**: merging PR #17 triggered the automatic worker deploy, which failed — `deploy-worker.yml` had the same `@quiz/shared` dist-not-built-yet gap as `ci.yml` (fixed in PR #17's own CI, but I missed the separate deploy workflow). Fixed in a follow-up PR #18 (`fix-deploy-worker-shared-build`), merged, and the redeployed worker succeeded. Production was briefly left on the pre-MILESTONE-05 build between the two merges; no user-facing impact since nothing depends on the new EventRoom behavior yet.
